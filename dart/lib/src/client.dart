...
class CitieslookupValidationException implements Exception {
  final List<String> errors;

  CitieslookupValidationException(this.errors);

  @override
  String toString() => 'CitieslookupValidationException: ${errors.join("; ")}';
}
...
/// Cities Lookup API client.
///
/// For more information, visit: https://apiverve.com/marketplace/citieslookup?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [city] (required) - The city name for which you want to get the data (e.g., New York)
/// * [limit] - Limit number of cities that match your search criteria [min: 1, max: 20]
class CitieslookupClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    ...
    'limit': ValidationRule(type: 'integer', required: false, min: 1, max: 20),
  };

  CitieslookupClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/citieslookup',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [CitieslookupValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw CitieslookupValidationException(errors);
    }
  }

  /// Execute a request to the Cities Lookup API.
  ///
  /// Parameters are validated before sending the request.
  Future<CitieslookupResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw CitieslookupException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return CitieslookupResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw CitieslookupException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw CitieslookupException('Resource not found');
      } else {
        throw CitieslookupException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is CitieslookupException) rethrow;
      throw CitieslookupException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Cities Lookup API client.
class CitieslookupException implements Exception {
  final String message;

  CitieslookupException(this.message);

  @override
  String toString() => 'CitieslookupException: $message';
}
